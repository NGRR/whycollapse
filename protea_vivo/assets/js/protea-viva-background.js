(() => {
  ...
  mergeConfig(CONFIG, window.PROTEA_VIVA_CONFIG || {});

  const TAU = Math.PI * 2;
  const canvas = document.querySelector("#protea-viva-canvas");
  const storyStart = document.querySelector("#colapso");
  const storyEnd = document.querySelector(".consolidation-footer");
  const storyGrid = storyStart?.querySelector(".consolidation-grid");
  const stage = canvas?.parentElement;
  if (!canvas || !storyStart || !storyEnd || !storyGrid || !stage) return;
  ...
  function resize() {
    ...
    state.mobile = matchMedia("(max-width: 959px)").matches;
  ...
  function updateScroll() {
    ...
    const start = storyStart.getBoundingClientRect().top + scrollY;
    const end = storyEnd.getBoundingClientRect().bottom + scrollY - innerHeight;
    state.targetProgress = clamp((scrollY - start) / Math.max(1, end - start));
    const active = scrollY + innerHeight * 0.15 > start && scrollY < end + innerHeight;
    if (active !== state.active) {
      state.active = active;
      document.body.classList.toggle("is-protea-viva-active", active);
  ...
  function updatePointer(event) {
    const rect = canvas.getBoundingClientRect();
    state.pointerTargetX = clamp((event.clientX - rect.left) / Math.max(1, rect.width) * 2 - 1, -1, 1);
    state.pointerTargetY = clamp((event.clientY - rect.top) / Math.max(1, rect.height) * 2 - 1, -1, 1);
  }
  ...
  function render(time) {
    ...
    if (state.active && !document.hidden) animationFrame = requestAnimationFrame(render);
    else animationFrame = 0;
  }

  function startRendering() {
    if (animationFrame || !state.active || document.hidden) return;
  ...
  function persistConfig() {
    try {
      localStorage.setItem("protea-viva-config-v16", JSON.stringify(CONFIG));
      return true;
    } catch {
      return false;
    }
  ...
  window.dispatchEvent(new CustomEvent("protea-viva-ready", { detail: window.ProteaVivaAnimator }));
  ...
  resize();
  loadTextures().finally(() => {
    document.documentElement.classList.add("protea-viva-is-ready");
    startRendering();
  });
})();
