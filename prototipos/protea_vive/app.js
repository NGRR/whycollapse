(() => {
  ...
  const DEFAULT_CONFIG = {
    ...
    growthLayers: [
      ...
      { path: "assets/stage-09-mature.webp", start: 0.66, peak: 0.82, end: 0.95, opacity: 0.13, scale: 1, yOffset: 0, drift: 1.5, kind: "mature" }
    ],
  ...
  function drawStrands(time, spin, geo, frontPass, alphaMultiplier = 1, mode = "all") {
    ...
    for (const spec of sorted) {
      ...
      ctx.strokeStyle = `rgba(255, ${52 + spec.brightness * 34}, 43, ${alphaDepth * reveal * (0.18 + spec.brightness * 0.24) * pulse * clarity * alphaMultiplier})`;
      ctx.lineWidth = spec.width * (isFront ? 1.0 : 0.68) * (0.70 + reveal * 0.54) * lerp(1, 0.88, cleaned);
